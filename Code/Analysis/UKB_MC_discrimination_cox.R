...
### Linear predictor function ####
    ...
    fun<-function(study_id){
      ...
      if (study_id=="pfr_13")                              {ds<-ds[which(ds$bilateral_oophorectomy_op==0 & ds$hysterectomy_prev==0),]} 
      ...
      model<-coxph(as.formula(paste0("Surv(study_time,",outcome,"_inc) ~ ",adjust_for)),ds)       # Cox model
...
write.csv(fig_data,paste0(path_7,"fig_data_cox_dis_main_na_obs1.csv"))             # eFigure 12

outcome<-"endometrial_cancer_x"                                                   # Broad outcome definition
fig_data<-lapply(study_id,fun)
fig_data<-as.data.frame(t(do.call(cbind, fig_data)))     
write.csv(fig_data,paste0(path_7,"fig_data_cox_dis_broad_na_obs1.csv"))            # eFigure 14
...
fig_data<-lapply(study_id,fun)
fig_data<-as.data.frame(t(do.call(cbind, fig_data)))     
write.csv(fig_data,paste0(path_7,"fig_data_cox_dis_main_0_obs1.csv"))               # eFigure 13

outcome<-"endometrial_cancer_x"                                                    # Broad outcome definition
fig_data<-lapply(study_id,fun)
fig_data<-as.data.frame(t(do.call(cbind, fig_data)))     
write.csv(fig_data,paste0(path_7,"fig_data_cox_dis_broad_0_obs1.csv"))              # eFigure 6
