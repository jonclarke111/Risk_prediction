...
write.csv(fig_data,paste0(path_7,"fig_data_log_dis_main_na_obs.csv"))                  # Not required

outcome<-"endometrial_cancer_x"                                                        # Broad outcome definition
fig_data<-lapply(study_id,fun)
fig_data<-as.data.frame(t(do.call(cbind, fig_data)))     
write.csv(fig_data,paste0(path_7,"fig_data_log_dis_broad_na_obs.csv"))                 # Not required
...
fig_data<-lapply(study_id,fun)
fig_data<-as.data.frame(t(do.call(cbind, fig_data)))     
write.csv(fig_data,paste0(path_7,"fig_data_log_dis_main_0_obs.csv"))                   # eFigure 11

outcome<-"endometrial_cancer_x"                                                        # Broad outcome definition
fig_data<-lapply(study_id,fun)
fig_data<-as.data.frame(t(do.call(cbind, fig_data)))     
write.csv(fig_data,paste0(path_7,"fig_data_log_dis_broad_0_obs.csv"))                  # Not required
