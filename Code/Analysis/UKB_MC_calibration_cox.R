...
### Linear predictor function ####
    ...
    fun<-function(study_id){
      ...
      model<-coxph(as.formula(paste0("Surv(study_time,",outcome,"_inc) ~ ",adjust_for)),ds)
...
write.csv(fig_data,paste0(path_7,"fig_data_cox_cal_main_na_obstt.csv"))                # Not required

outcome<-"endometrial_cancer_x"                                                        # Broad outcome definition
fig_data<-lapply(study_id,fun)
fig_data<-as.data.frame(t(do.call(cbind, fig_data)))     
write.csv(fig_data,paste0(path_7,"fig_data_cox_cal_broad_na_obs.csv"))                 # Not required
...
fig_data<-lapply(study_id,fun)
fig_data<-as.data.frame(t(do.call(cbind, fig_data)))     
write.csv(fig_data,paste0(path_7,"fig_data_cox_cal_main_0_obs.csv"))                   # Not required 

outcome<-"endometrial_cancer_x"                                                        # Broad outcome definition
fig_data<-lapply(study_id,fun)
fig_data<-as.data.frame(t(do.call(cbind, fig_data)))     
write.csv(fig_data,paste0(path_7,"fig_data_cox_cal_broad_0_obs.csv"))                  # Not required
